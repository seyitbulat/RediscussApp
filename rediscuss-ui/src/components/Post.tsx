                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    {(() => {
                                        ...
                                        const topLevel = comments.filter((cc: any) => !cc.parentCommentId);
                                        // If there are no top-level comments but there are comments (only replies),
                                        // render all comments as a fallback so replies are visible.
                                        const displayList = topLevel.length > 0 ? topLevel : comments;
                                        return displayList.map((c: any) => (
                                            <div key={c.id}>
                                                <div className="rounded-md border p-3 bg-surface/50">
                                                    <div className="text-sm font-medium">{c.createdByUsername}</div>
                                                    <div className="text-sm text-text-500 mt-1">{c.content}</div>
                                                    <div className="text-xs text-text-300 mt-1">{formatDistanceToNow(new Date(c.createdAt), { addSuffix: true, locale: tr }).replace("yaklaşık", "")}</div>
                                                    <div className="mt-2 flex items-center gap-2">
                                                        <Button size="icon" variant="ghost" onClick={() => setReplyToId(c.id)}>
                                                            Yanıtla
                                                        </Button>
                                                    </div>
                                                        ...
                                                        <div className="mt-2">
                                                            ...
                                                            <div className="flex gap-2 justify-end mt-1">
                                                                ...
                                                                <Button
                                                                    ...
                                                                    onClick={() => commentMutation.mutate({ text: replyText.trim(), parentCommentId: c.id })}
                                                                >Gönder</Button>
                                                            </div>
                                                        </div>
                                                    )}
                                                ...
                                                <div className="pl-6 mt-2 flex flex-col gap-2">
                                                    {comments.filter((r: any) => r.parentCommentId === c.id).map((r: any) => (
                                                        <div key={r.id} className="rounded-md border p-3 bg-surface/50">
                                                            <div className="text-sm font-medium">{r.createdByUsername}</div>
                                                            <div className="text-sm text-text-500 mt-1">{r.content}</div>
                                                            <div className="text-xs text-text-300 mt-1">{formatDistanceToNow(new Date(r.createdAt), { addSuffix: true, locale: tr }).replace("yaklaşık", "")}</div>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
